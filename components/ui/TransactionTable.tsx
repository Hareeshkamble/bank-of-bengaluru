...
export default function TransactionTable({transactions}:TransactionTableProps) {
  ...
  <TableHeader className='bg-[#f9fafb]'>
    <TableRow>
      ...
      <TableHead className='px-2 mx-md:hidden'>Channel</TableHead>
      <TableHead className='px-2 mx-md:hidden'>Category</TableHead>
...
{transactions.map((t:Transaction)=>{
    ...
    return(
        ...
        !over:bg-none !border-b-DEFAULT`}>
            <TableCell className='mzx-w-[250px] pl-2 pr-10'>
                <div className='flex items-center gap-3'>
                    <h1 className='text-14 truncate text-[#344054]'>{removeSpecialCharacters(t.name)}</h1>
                </div>
            </TableCell>
